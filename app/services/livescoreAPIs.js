...
export function getMatchDetails(category, eventID, p) {
  //   const requestURL = `https://livescore6.p.rapidapi.com/matches/detail?p=1&category=soccer&id=3081629`;
  console.log(
    "getMatchDetailsAPI method category, eventID, p",
    category,
    eventID,
    p
  );
  const requestURL = `${host}/matches/detail?p=${p}&category=${category}&id=${eventID}`;
  const queryString = null;
  const options = {
    method: "GET",
    headers: {
      "x-rapidapi-host": "livescore6.p.rapidapi.com",
      "x-rapidapi-key": "fd14061a4fmsh7c509c1cb907936p14bc0cjsn90bc793721cd",
    },
  };
